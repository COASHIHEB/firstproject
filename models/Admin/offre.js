    ...
    static addOffre(inputs, CallBack) {
        ...
        connexion.query("SELECT idSousCat AS id from souscategorie WHERE nom = ?", [categorie], (err, resultat) => {
            if (err) throw err;
                ...
                connexion.query("INSERT INTO offre (nom, description, prix, dure, date, SousCategorie_idSousCat) VALUES (?,?,?,?,?,?)", [input.nom, input.description, input.prix, input.dure, date, idSC], (err, result) => {
                    if (err) {
                        throw err;
                    } else {
