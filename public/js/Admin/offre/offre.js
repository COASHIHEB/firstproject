  ...
  $.post("SelectPhoto",
    ...
    function (data, status) {
      ...
      if (data != 'error') {
        ...
        let caroussel = '<div class="carousel-inner" id="photo">'
        for (let i = 0; i < data.length; i++) {
          if (i == 0) {
...
function recupereSousCat(id) {
  $('option:checked[name=sous' + id + ']').each(function () {
    sousCategorieModif = $(this).val(); console.log(sousCategorieModif)
  });
}


function selCatModif(id) {
  var categorie = $('#categorieModif' + id).val();
  $.post("selectSousCat", {
    categorie: categorie,
  },
    function (data, status) {
      if (data != 'error') {
        let toReplac = '<div class="col-sm-3"  id="modif' + id + '"><select class="chercheBarDropDown form-control"  style="width: auto;">';
        for (let i = 0; i < data.length; i++) {
