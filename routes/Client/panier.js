...
app.post('/panier', (request, response) => {
    if (request.session.userId) {
        require("../../models/Client/panier").selectPanier(request.session.userId, (resp) => {
            response.json({ commandes: resp.commandes, devis: resp.devis });
        });
    } else {
...
app.get('getNumberOfItem'), (request, response) => {
    if (request.session.userId) {
        require("../../models/Client/panier").getNumberOfItem(request.body, (resp) => {
            response.json(resp);
        });
    } else {
        response.json("notConnected");
